...
namespace regex {
    ...
    public:
        static std::expected<Pcre2Private *, std::string> generate(std::string pattern) {
            int errornumber;
            PCRE2_SIZE erroroffset;
            pcre2_code *re = pcre2_compile(
                reinterpret_cast<PCRE2_SPTR>(pattern.data()), /* the pattern */
                PCRE2_ZERO_TERMINATED,                        /* indicates pattern is zero-terminated */
                PCRE2_UTF | PCRE2_NEWLINE_ANYCRLF,            /* default options */
            ...
            if (re == NULL) {
                ...
                return std::unexpected(std::format("PCRE2 compilation failed at offset %d: %s", erroroffset, (char *)buffer));
            }
            pcre2_match_data *match_data = pcre2_match_data_create_from_pattern(re, NULL);
            return new Pcre2Private(re, match_data);
        };

        std::expected<bool, std::string> find(std::string subject, PCRE2_SIZE start) { /* Now run the match. */
            int rc = pcre2_match(
            ...
            if (rc < 0) {
                ...
                return std::unexpected(std::format("PCRE2 matching failed: %s", (char *)buffer));
            }
            return true;
        }

        std::expected<std::string, std::string> replace(std::string subject, std::string replacementText, int replaceTimes, size_t start) {
            // 启用扩展替换处理，将未知的命名捕获组视为未设置，简单的未设置插入等于空字符串
            uint32_t options = PCRE2_SUBSTITUTE_EXTENDED | PCRE2_SUBSTITUTE_UNKNOWN_UNSET | PCRE2_SUBSTITUTE_UNSET_EMPTY;
            if (replaceTimes == 0) {
                options = options | PCRE2_SUBSTITUTE_GLOBAL;
                replaceTimes++;
            }
            PCRE2_SIZE bufferSize = maxBufferSize;
            pcre2_substitute(re,                                          // 编译后的正则表达式
    ...
    private:
        Pcre2Private(pcre2_code *re, pcre2_match_data *match_data) : re(re), match_data(match_data), maxBufferSize(256), buffer(std::make_unique<PCRE2_UCHAR>(maxBufferSize)) {
        };
        // 禁止外部拷贝构造
...
regex::Pcre2Implementation::Pcre2Implementation(std::string pattern) : pattern(pattern), d(nullptr) {
}

regex::Pcre2Implementation::~Pcre2Implementation() {
}

std::expected<bool, std::string> regex::Pcre2Implementation::find(std::string text, size_t start) {
    auto result = init();
    if (result.value_or(false)) {
        return d->find(text, start);
    }
    return std::unexpected(result.error_or("初始化错误！"));
}

std::expected<std::string, std::string> regex::Pcre2Implementation::replace(std::string originalText, std::string replacementText, int replaceTimes, size_t start) {
    auto result = init();
    if (result.value_or(false)) {
        return d->replace(originalText, replacementText, replaceTimes, start);
    }
    return std::unexpected(result.error_or("初始化错误！"));
}
